...
#include "../utils/headers/GameData.h"
...
Alien::Alien(GameObject& associated, int nMinions, float timeOffset)
    : Being(associated, 30), nMinions(nMinions), state(RESTING), timeOffset(timeOffset) {
...
void Alien::Update(float dt) {
    switch (state) {
        case RESTING:
            if (restTimer.Get() >= COOLDOWN_TIME + timeOffset) {
                restTimer.Restart();
    ...
    if (hp <= 0) {
        ...
        Sprite* sprite = new Sprite(*go, "assets/img/aliendeath.png", 4, .125, .5);
        ...
        *new PostDeletionAction(*go) += make_pair(
            new Action([sprite, sound]() {
                ...
                if (alienCount == 0) GameData::playerVictory = true;
            }),
